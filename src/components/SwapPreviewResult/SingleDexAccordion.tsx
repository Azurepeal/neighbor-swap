...
import { tokenOutAtom } from 'src/domain/swap/atom';
import { QuoteResponseDto } from 'src/types/quote';
import getBigNumberWithDecimal from 'src/utils/get-denom-big-number';
                  ...
                  <ListItem key={item.dexId}>
                    <HStack justifyContent="space-between">
                      ...
                      <Text fontSize={['sm', 'md', 'md', 'md']}>{`${withComma(
                        getBigNumberWithDecimal(
                          item.expectedAmountOut,
                          tokenOut ? -tokenOut.decimals : -1,
                        ).toString(),
                        3,
